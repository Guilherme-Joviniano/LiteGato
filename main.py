...
def actual_newsletter(update, context):
    ...
    context.bot.send_message(chat_id = get_chat_id(update, context),text = 'Essa semana o tema é Bird Box, de Josh Malerman, um autor fantástico do gênero horror. ')
    context.bot.send_message(chat_id = get_chat_id(update, context),text = 'A obra também tem um filme homônimo, que você muito provavelmente já assistiu (aquele com a Sandra Bullock), e, embora sigam a mesma linha, acho que alguns detalhes que foram deixados de lado fazem a diferença... \n Aqui, o link do trailer do filme, de 2018, e o link para o livro, de 2014. ')
    context.bot.send_message(chat_id = get_chat_id(update, context),text ='https://www.estantevirtual.com.br/livros/josh-malerman/caixa-de-passaros-nao-abra-os-olhos/2582672683?livro_usado=1&b_order=preco&gclid=CjwKCAjwz5iMBhAEEiwAMEAwGDH42vMyDY5CV-4pyvwPytzecikNLXsHr938YhP5VOLGI4btJWOGLhoC0uwQAvD_BwE')
    context.bot.send_message(chat_id = get_chat_id(update, context),text ='https://www.youtube.com/watch?v=o2AsIXSh2xo&ab_channel=Netflix')
    context.bot.send_message(chat_id = get_chat_id(update, context),text = 'Mas, se você ainda não conhece nada desse universo de terror do Josh Malerman, esse é um livro daqueles em que você se arrepia lendo, mesmo sem nada estar acontecendo: você sente a tensão das personagens como se estivesse junto com eles. Admito que, mesmo lendo o livro depois de ver o filme, e sabendo o que aconteceria, tive pesadelos.')
    context.bot.send_message(chat_id = get_chat_id(update, context),text = 'A história intercala o passado pré-apocalipse e o presente surto de criaturas - que, ao serem vistas, tornam as pessoas extremamente violentas e as levam a um suposto suicídio. Acompanhamos esse mundo pela vida de Malorie e seus dois filhos, que entram em uma aventura em busca de um abrigo.')
    context.bot.send_animation(chat_id = get_chat_id(update, context), animation = 'https://media3.giphy.com/media/3de2L6nG69pGN8qsha/giphy.gif?cid=ecf05e47jc2o50dcquk51lafrjjmh2nfd6ddcsbh8qg1v7k6&rid=giphy.gif&ct=g')     
    context.bot.send_message(chat_id = get_chat_id(update, context),text ='Acho legal começar falando de como a irmã da Malorie – a protagonista – é pouco explorada no filme em comparação ao livro, inclusive recebendo um nome diferente (a Netflix resolveu renomear a Shannon como Jessica, o que particularmente não curti). No livro, as  irmãs mantêm uma longa quarentena, e é dentro do banheiro da própria casa que Shannon tem contato com a criatura e morre. No filme, isso é um pouco mais agressivo. Outras personagens também têm mudanças significativas na adaptação cinematográfica da Netflix, mas essa foi a que mais me incomodou...')
    context.bot.send_animation(chat_id = get_chat_id(update, context), animation = 'https://media4.giphy.com/media/NSXTBM0scrrG1Kq41E/giphy.gif?cid=ecf05e47xa3hmkb63pysypxm6g4l34jicn07icv2h8i4xop2&rid=giphy.gif&ct=g')     
    context.bot.send_message(chat_id = get_chat_id(update, context),text ='Agora, meu ponto preferido: os animais, especificamente os pássaros e os cachorros. No caso do filme, apenas os pássaros são utilizados como sistema de alarme, avisando os sobreviventes da presença sobrenatural. No livro, temos, junto deles, os cachorros, e descobrimos mais detalhes sobre como isso afeta eles.')
    context.bot.send_animation(chat_id = get_chat_id(update, context), animation = 'https://media0.giphy.com/media/5z5KkOi30FKiFjXUnx/giphy.gif?cid=ecf05e47xa3hmkb63pysypxm6g4l34jicn07icv2h8i4xop2&rid=giphy.gif&ct=g')
    context.bot.send_message(chat_id = get_chat_id(update, context), text = 'ATENÇÃO: SPOILER!!!')
    context.bot.send_message(chat_id = get_chat_id(update, context), text = 'Você foi avisado que esse parágrafo conteria spoilers sobre o final do filme e do livro.')
    context.bot.send_message(chat_id = get_chat_id(update,context), text = 'Embora o final da história seja muito semelhante nas duas obras, no livro tem um ar mais pesado que foi deixado de fora do filme. Quando Malorie chega à escola para cegos, ela descobre que alguns dos sobreviventes tiraram a própria visão e, a princípio, teme que o mesmo aconteça com ela. E mesmo chegando a pensar em fazer isso com as crianças, após receber conselhos de Don, desiste.')
    context.bot.send_animation(chat_id = get_chat_id(update, context), animation = 'https://media2.giphy.com/media/5tujE5k9bBte00XW3M/giphy.gif?cid=ecf05e47xa3hmkb63pysypxm6g4l34jicn07icv2h8i4xop2&rid=giphy.gif&ct=g')
    context.bot.send_message(chat_id = get_chat_id(update, context), text = 'Mari Indica!') 
    context.bot.send_message(chat_id = get_chat_id(update, context),text ='Pois ééé, hoje o livro foi mais tenso, e isso porque quase todas as obras do autor têm a mesma pegada sobrenatural, que te faz sentir o ambiente por completo e arrepia até o último fio de cabelo. Se você gosta desse tipo de leitura, super recomento “Uma casa no fundo de um lago”, do mesmo autor, e “Piano Vermelho”, também dele. Confira nos links!')
...
def handle_message(update,context):
    ...
    if user_messages in('/quinta', 'quinta'):
          ...
          context.bot.send_message(chat_id = get_chat_id(update, context), text = TextoDoFim)
    if user_messages in ('sexta','/sexta'):
